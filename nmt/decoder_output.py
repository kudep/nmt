...
class OutputDecoder(core.Dense):
  ...
  def __init__(self, units,
               ...
               embeddings_units=None,
               **kwargs):
    super(OutputDecoder, self).__init__(units = units,trainable=trainable, name=name, **kwargs)
    self.units = units
    ...
    self.embeddings_units = embeddings_units
  ...
  def call(self, inputs):
    ...
    output_shape = shape[:-1] + [self.embeddings_units]
    ...
    #                                                            [0]])
      ...
      projection = standard_ops.matmul(inputs, self.kernel)
      output_embeddins = standard_ops.transpose(self.decoder_embeddings)
      outputs = standard_ops.matmul(projection, output_embeddins)
    if self.use_bias:
  ...
  def _compute_output_shape(self, input_shape):
    ...
    if input_shape[-1].value is None:
      raise ValueError(
          ...
          % input_shape)
    return input_shape[:-1].concatenate(self.embeddings_units)
