...
def ext_add(parser,model_dir,ckpt):
    parser.set_defaults(out_dir = model_dir)
    parser.set_defaults(ckpt = ckpt)
    parser.set_defaults(inference_output_file = inf_output_file)
    parser.set_defaults(inference_input_file = inf_input_file)
...
def init(model_dir, ckpt_name = None, best_bleu = False, _context_len=5, _man_context_len=1):
  ...
  FLAGS = params['flags']
  # assert False
  params['default_hparams'] = create_hparams(params['flags'])
